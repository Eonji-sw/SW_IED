...
float dist_half, percent_bright, bright;
...
// turn on the LED if the distance is between dist_min and dist_max  
  ...
  else if(dist_raw <= dist_half) {
    percent_bright = (dist_raw - dist_min) * 100 / (dist_half - dist_min);
    bright = 256 - (256 * (percent_bright / 100));
    //Serial.println(percent_bright);
    //Serial.println(bright);
    analogWrite(PIN_LED, bright);
  }
  else if(dist_raw > dist_half) {
    percent_bright = (dist_max - dist_raw) * 100 / (dist_max - dist_half);
    bright = 256 - (256 * (percent_bright / 100));
    //Serial.println(percent_bright);
    //Serial.println(bright);
    analogWrite(PIN_LED, bright);
